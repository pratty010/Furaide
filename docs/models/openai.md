...
- `high` — novel problems, architecture decisions, adversarial review, legal/compliance. Default for `@strategist:ARCHITECT`, `@strategist:PLAN`, `@reviewer`, `@legal`, `@synthesizer`.
- `medium` — multi-file codegen, research synthesis, moderate debugging. Default for `@coder`, `@debugger`.
- `minimal` — scan/parse/extract, boilerplate, format transforms. Use for `@scanner` fallback when on gpt-5.x. Pair with tool preambles (see below).
