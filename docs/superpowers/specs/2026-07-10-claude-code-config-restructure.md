...
**Code-review fixes (post-implementation, 2026-07-10) — an independent review pass on the committed diff found 5 real Important/Critical issues, all fixed and re-verified with a full install→uninstall round trip before merge:**
1. **Unguarded network step could abort the whole install.** `install.sh`'s new default-on external-skills step (a `git clone`/`pull`) ran under `set -euo pipefail` with no error handling — a transient network failure would abort CLAUDE.md/rules/agents/settings installation too, none of which depend on it. Fixed: both branches now `&& ok ... || warn ...`, non-fatal.
2. **`research` skill's reinstall check had the symlink-vs-real-dir logic backwards**, misfiring a "foreign file" warning on every ordinary re-run (since our own install is always a real `cp -r`, never a symlink) while failing to actually supersede the external stub it claimed to. Fixed: simplified to the same skip-if-exists pattern already used for `config/agents/*.md` elsewhere in the same file — no more symlink branching, no more wrong claim in the comment.
3. **`install-external-skills.sh --all` pulled in more than the canonical set, redundantly self-cloning this monorepo.** The exclusion filter targeted `install_target == "manual"`, but the actual culprit (`hanko`/`github`, already covered by the vendored-pool path) is tagged `"common"`, not `"manual"` — confirmed by an E2E test that still showed a `friday` (this repo) clone after the first fix attempt. Corrected to a positive allow-list (`install_target == "global"`, the only value that actually means "auto-install via this script") instead of a deny-list — verified via a second E2E round: `skill-repos/` now contains exactly `superpowers` + `ponytail`, no self-clone.
4. **Project-scope CC installs didn't isolate the external-skills step**, mirroring the exact custom-scope bug already found and fixed (see above) — `--project` alone doesn't pass a path, so it wrote to `$PWD`-relative `./.claude/skills` regardless of where `TARGET_DIR` actually resolved to. Fixed: unified custom AND project scope onto the same `AGENTS_SKILL_REPOS`/`AGENTS_SKILLS`/`CLAUDE_SKILLS` env-var redirection (only `global` scope skips the override now).
5. **The "Core skills" install prompt undersold its own scope** — answering `n`/`none` silently also skipped the bundled `research` skill and the entire canonical external set, but the prompt text only named the 5 `CC_SKILL_SET` repo skills. Fixed: prompt text now names all three groups.

One additional fix caught by the same E2E testing, not from the review: `notebooklm` and the README/uninstall.sh's canonical-skill list were left overclaiming notebooklm auto-installs (it's still correctly `install_target: "manual"`, hand-install-only) — corrected in `harnesses/claude-code/README.md` and `uninstall.sh`'s cleanup list to match.
