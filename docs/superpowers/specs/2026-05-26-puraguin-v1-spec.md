...
Single binary `puraguin` (Python; richest SDK ecosystem for SQLite + LLM APIs). Sub-commands map 1:1 to phases. Each phase is independently runnable and idempotent.
...
```
puraguin ingest                          # phase A
puraguin judge [--reanalyze SKILL|all]   # phase B
puraguin aggregate                       # phase C
puraguin improve [--skill SKILL]         # phase D
puraguin run                             # A → B → C in sequence
puraguin report --overview               # phase E (see Layer 3)
puraguin report --skill SKILL [--deep]   # phase E
puraguin reset --from PHASE              # wipe checkpoints + re-run from phase
```
...
## Layer 3 — `/puraguin` skill (user-facing)

Claude Code skill bundled with the plugin. Invoked as `/puraguin` from any Claude Code session.
...
├── skills/
│   └── puraguin/
│       └── SKILL.md               # /puraguin user-facing skill
├── cli/
│   ├── pyproject.toml
│   └── puraguin/
│       ├── __main__.py
...
Everything is greenfield. The repo at `/d/Everything/Experimental/Puraguin/` is empty. All files in the layout above are new.
