...
**Architecture:** Three layers. (1) Capture: bash hooks emit JSONL events to `~/.puraguin/events/claude-code/YYYY-MM-DD.jsonl` — append-only, ≤10ms hot path. (2) Analyzer: Python CLI with four idempotent phases (Ingest → Judge → Aggregate → Improve), checkpointed via SQLite at `~/.puraguin/state.db`. (3) `/puraguin` skill renders HTML reports via `python3 -m http.server` and hands off skill rewrites to `skill-creator` / `writing-skills`. Transcripts at `~/.claude/projects/<encoded-cwd>/<session>.jsonl` are the source of truth; events are pointers. Compaction is non-destructive on disk (verified empirically), so no hot-path file copies are needed.
...
- `/puraguin improve <skill>` correctly hands off to `Skill(skill-creator)` / `Skill(writing-skills)` rather than rewriting inline.
