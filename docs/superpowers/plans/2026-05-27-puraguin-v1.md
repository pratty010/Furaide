# Puraguin v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build Puraguin v1 — a Claude Code plugin that captures every skill invocation with surrounding context, runs offline LLM-as-judge analysis with incremental checkpointing, and surfaces per-skill HTML reports plus skill-improvement evidence packs.

**Architecture:** Three layers. (1) Capture: bash hooks emit JSONL events to `~/puraguin/events/claude-code/YYYY-MM-DD.jsonl` — append-only, ≤10ms hot path. (2) Analyzer: Python CLI with four idempotent phases (Ingest → Judge → Aggregate → Improve), checkpointed via SQLite at `~/puraguin/state.db`. (3) `/puraguin` skill renders HTML reports via `python3 -m http.server` and hands off skill rewrites to `skill-creator` / `writing-skill`. Transcripts at `~/.claude/projects/<encoded-cwd>/<session>.jsonl` are the source of truth; events are pointers. Compaction is non-destructive on disk (verified empirically), so no hot-path file copies are needed.
...
**Spec reference:** `/home/ace/.claude/plans/we-are-starting-happy-locket.md`.
...
│   │   └── puraguin/
│   │       ├── __init__.py
│   │       ├── __main__.py               # `python -m puraguin`
│   │       ├── cli.py                    # argparse dispatch
│   │       ├── config.py                 # config loader (~/puraguin/config.toml)
...
│       └── test_cli.py
├── bin/
│   └── puraguin                          # shim → python -m puraguin
├── docs/
│   └── superpowers/
...
- `~/puraguin/events/claude-code/YYYY-MM-DD.jsonl` — raw event log
- `~/puraguin/state.db` — SQLite analyzer state
- `~/puraguin/evidence/<skill>-<date>.md` — Phase D evidence packs
- `~/puraguin/reports/<ts>-<topic>.html` — Layer 3 reports
- `~/puraguin/config.toml` — user config (judge backend, sample sizes, etc.)
...
**Files:**
- Create: `/d/Everything/Experimental/Puraguin/.gitignore`
- Create: `/d/Everything/Experimental/Puraguin/README.md`
- Create: `/d/Everything/Experimental/Puraguin/docs/superpowers/specs/2026-05-26-puraguin-v1-spec.md` (copy of spec)
...
```bash
cp /home/ace/.claude/plans/we-are-starting-happy-locket.md \
   /d/Everything/Experimental/Puraguin/docs/superpowers/specs/2026-05-26-puraguin-v1-spec.md
```

- [ ] **Step 4: Commit**

```bash
cd /d/Everything/Experimental/Puraguin
...
```json
{
  ...
  "author": {
    "name": "puraguin",
    "url": "https://github.com/pratty010/puraguin"
  }
}
```
...
```bash
mkdir -p ~/.claude/plugins/puraguin-dev
ln -sfn /d/Everything/Experimental/Puraguin ~/.claude/plugins/puraguin-dev/source
claude plugin details puraguin-dev 2>&1 | head -20
```
...
- Output directory: `${PURAGUIN_HOME:-$HOME/puraguin}/events/claude-code/YYYY-MM-DD.jsonl`.
...
```bash
PURAGUIN_HOME=/tmp/puraguin-test bash -c '
  source hooks/_emit.sh <<< ""
' 2>&1 || true
PURAGUIN_HOME=/tmp/puraguin-test ./hooks/_emit.sh '{"ts":"2026-05-27T00:00:00.000Z","event":"test","x":1}'
cat /tmp/puraguin-test/events/claude-code/$(date -u +%Y-%m-%d).jsonl
```
...
```bash
PURAGUIN_HOME=/tmp/puraguin-test echo '{
  "session_id":"abc","cwd":"/tmp","model":"claude-opus-4-7","source":"startup","transcript_path":"/tmp/t.jsonl"
}' | ./hooks/session_start.sh
tail -1 /tmp/puraguin-test/events/claude-code/$(date -u +%Y-%m-%d).jsonl | jq .
```
...
```bash
LINES_BEFORE=$(wc -l < /tmp/puraguin-test/events/claude-code/$(date -u +%Y-%m-%d).jsonl)
PURAGUIN_HOME=/tmp/puraguin-test echo '{"tool_name":"Bash","session_id":"x"}' | ./hooks/skill_pre.sh
LINES_AFTER=$(wc -l < /tmp/puraguin-test/events/claude-code/$(date -u +%Y-%m-%d).jsonl)
...
tail -2 /tmp/puraguin-test/events/claude-code/$(date -u +%Y-%m-%d).jsonl | jq .
```
...
```bash
claude plugin details puraguin-dev 2>&1 | grep -E "Hooks|hook"
```
...
- All file paths derived from `puraguin.paths` to keep ~/puraguin location overridable in tests via `PURAGUIN_HOME`.
...
- Create: `cli/src/puraguin/__init__.py`
- Create: `cli/src/puraguin/__main__.py`
...
```python
# cli/src/puraguin/__main__.py
from puraguin.cli import main
...
def test_package_importable():
    import puraguin
    assert puraguin.__version__ == "0.1.0"
```

- [ ] **Step 6: Install in editable mode and run tests**

```bash
cd /d/Everything/Experimental/Puraguin/cli
python -m venv .venv
...
**Files:**
- Create: `cli/src/puraguin/paths.py`
- Create: `cli/src/puraguin/config.py`
- Create: `cli/tests/test_paths.py`

- [ ] **Step 1: Write `paths.py`**

```python
# cli/src/puraguin/paths.py
import os
from pathlib import Path
...
```bash
cd /d/Everything/Experimental/Puraguin/cli && .venv/bin/pytest -v
```
...
```bash
git add cli/src/puraguin/paths.py cli/src/puraguin/config.py cli/tests/test_paths.py
...
```bash
cd /d/Everything/Experimental/Puraguin/cli && .venv/bin/pytest tests/test_db.py -v
```

Expected: FAIL (`puraguin.db` does not exist).

- [ ] **Step 3: Write `db.py`**

```python
# cli/src/puraguin/db.py
import sqlite3
...
```bash
git add cli/src/puraguin/db.py cli/tests/test_db.py
...
**Files:**
- Create: `cli/src/puraguin/events.py`
...
from puraguin import events, paths
from puraguin.paths import events_dir

def test_iter_events_reads_all_jsonl_lines(puraguin_home):
    d = events_dir()
...
def test_iter_events_respects_checkpoints(puraguin_home):
    from puraguin import db
    db.init()
    d = events_dir()
...
```bash
git add cli/src/puraguin/events.py cli/tests/test_events.py
...
**Files:**
- Create: `cli/src/puraguin/transcript.py`
...
from pathlib import Path
from puraguin import transcript
...
```python
# cli/src/puraguin/transcript.py
import json
...
```bash
git add cli/src/puraguin/transcript.py cli/tests/fixtures/ cli/tests/test_transcript.py
...
**Files:**
- Modify: `cli/src/puraguin/transcript.py` (add function)
...
```python
# append to cli/src/puraguin/transcript.py
import re
...
```bash
git add cli/src/puraguin/transcript.py cli/tests/fixtures/transcript_with_skills_reminder.jsonl cli/tests/test_transcript.py
...
**Files:**
- Create: `cli/src/puraguin/ingest.py`
...
```bash
git add cli/src/puraguin/ingest.py cli/tests/test_ingest.py
...
**Files:**
- Create: `cli/src/puraguin/cli.py`
...
def test_cli_ingest_runs(puraguin_home, tmp_path):
    f = puraguin_home / "events" / "claude-code" / "2026-05-27.jsonl"
    f.parent.mkdir(parents=True, exist_ok=True)
    ...
    r = subprocess.run(
        [sys.executable, "-m", "puraguin", "ingest"],
        env={"PURAGUIN_HOME": str(puraguin_home), "PATH": "/usr/bin:/bin"},
...
```bash
git add cli/src/puraguin/cli.py cli/tests/test_cli.py
...
**Files:**
- Create: `cli/src/puraguin/judge/__init__.py`
- Create: `cli/src/puraguin/judge/interface.py`

- [ ] **Step 1: Write `__init__.py`**

```python
# cli/src/puraguin/judge/__init__.py
from puraguin.judge.interface import (
...
```python
# cli/src/puraguin/judge/interface.py
from dataclasses import dataclass
...
```bash
git add cli/src/puraguin/judge/__init__.py cli/src/puraguin/judge/interface.py
...
**Files:**
- Create: `cli/src/puraguin/judge/context.py`
...
from puraguin import db, ingest
...
```python
# cli/src/puraguin/judge/context.py
from pathlib import Path
...
```bash
git add cli/src/puraguin/judge/context.py cli/tests/test_judge_context.py
...
**Files:**
- Create: `cli/src/puraguin/judge/prompts.py`

- [ ] **Step 1: Write `prompts.py`**

```python
# cli/src/puraguin/judge/prompts.py
...
```bash
git add cli/src/puraguin/judge/prompts.py
...
**Files:**
- Create: `cli/src/puraguin/judge/anthropic_backend.py`

- [ ] **Step 1: Write `anthropic_backend.py`**

```python
# cli/src/puraguin/judge/anthropic_backend.py
import json
import os
...
```bash
git add cli/src/puraguin/judge/anthropic_backend.py
...
**Files:**
- Create: `cli/src/puraguin/judge/codex_backend.py`

- [ ] **Step 1: Write `codex_backend.py`**

```python
# cli/src/puraguin/judge/codex_backend.py
import json
import subprocess
from puraguin.judge.interface import (
    InvocationContext, Judgment, Gap, AvailableSkillCtx
...
```bash
git add cli/src/puraguin/judge/codex_backend.py
...
**Files:**
- Create: `cli/src/puraguin/judge/orchestrator.py`
...
from puraguin.judge.interface import Judgment, Gap
...
```python
# cli/src/puraguin/judge/orchestrator.py
from datetime import datetime, timezone
from pathlib import Path
from puraguin import db, transcript
...
```bash
git add cli/src/puraguin/judge/orchestrator.py cli/tests/test_judge_orchestrator.py
...
**Files:**
- Modify: `cli/src/puraguin/cli.py`

- [ ] **Step 1: Add subcommand**

```python
# append to cli/src/puraguin/cli.py (inside build_parser, before `return p`)
...
```bash
git add cli/src/puraguin/cli.py
...
**Files:**
- Create: `cli/src/puraguin/aggregate.py`
...
```bash
git add cli/src/puraguin/aggregate.py cli/tests/test_aggregate.py cli/src/puraguin/cli.py
...
**Files:**
- Create: `cli/src/puraguin/improve.py`
...
from puraguin import db, improve
...
def test_evidence_pack_generated(puraguin_home):
    db.init()
    conn = db.connect()
    _seed(conn)
    conn.close()
...
def test_mark_status(puraguin_home):
    db.init()
    conn = db.connect()
    _seed(conn)
    conn.close()
...
def test_insufficient_sample_raises(puraguin_home):
    db.init()
    try:
...
```bash
git add cli/src/puraguin/improve.py cli/tests/test_improve.py cli/src/puraguin/cli.py
git commit -m "feat(cli): Phase D evidence-pack builder + handoff doc"
```

---

## Milestone 7 — Layer 3 (`/puraguin` skill + HTML reports)

### Task 7.1: HTML templates (overview + per-skill)

**Files:**
- Create: `cli/src/puraguin/report/__init__.py`
- Create: `cli/src/puraguin/report/templates/base.html`
- Create: `cli/src/puraguin/report/templates/overview.html`
- Create: `cli/src/puraguin/report/templates/skill_detail.html`

- [ ] **Step 1: Empty `__init__.py`**

```python
# cli/src/puraguin/report/__init__.py
```

- [ ] **Step 2: `base.html`**

```html
{# cli/src/puraguin/report/templates/base.html #}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>{{ title }} — Puraguin</title>
<style>
...
```html
{# cli/src/puraguin/report/templates/overview.html #}
{% extends "base.html" %}
{% block content %}
...
```html
{# cli/src/puraguin/report/templates/skill_detail.html #}
{% extends "base.html" %}
{% block content %}
...
```bash
git add cli/src/puraguin/report/__init__.py cli/src/puraguin/report/templates/
...
**Files:**
- Create: `cli/src/puraguin/report/generate.py`
...
from puraguin import db
from puraguin.report import generate
...
def test_overview_renders(puraguin_home):
    db.init()
...
def test_skill_detail_renders(puraguin_home):
    db.init()
    conn = db.connect(); _seed(conn); conn.close()
...
```bash
git add cli/src/puraguin/report/generate.py cli/tests/test_report.py
...
**Files:**
- Create: `cli/src/puraguin/report/server.py`
...
```bash
git add cli/src/puraguin/report/server.py
...
### Task 7.5: `/puraguin` user-facing skill

**Files:**
- Create: `skills/puraguin/SKILL.md`
...
```bash
git add skills/puraguin/SKILL.md
git commit -m "feat(skill): /puraguin user-facing skill with handoff rules"
```
...
ln -sfn /d/Everything/Experimental/Puraguin ~/.claude/plugins/puraguin
```

- [ ] **Step 2: Verify Claude Code sees all components**

```bash
claude plugin details puraguin 2>&1
```

Expected: lists Skills (1: puraguin), Hooks (7), Agents (0).

- [ ] **Step 3: Ensure all hook scripts are executable**

```bash
chmod +x /d/Everything/Experimental/Puraguin/hooks/*.sh
```

- [ ] **Step 4: Install the CLI globally so `puraguin` is on PATH**

```bash
cd /d/Everything/Experimental/Puraguin/cli
.venv/bin/pip install -e .
ln -sfn /d/Everything/Experimental/Puraguin/cli/.venv/bin/puraguin ~/.local/bin/puraguin
puraguin --help
```
...
```bash
mkdir -p /tmp/puraguin-smoke && cd /tmp/puraguin-smoke
claude
```
...
```bash
cat ~/puraguin/events/claude-code/$(date -u +%Y-%m-%d).jsonl | jq -c '.event' | sort | uniq -c
```
...
```bash
sqlite3 ~/puraguin/state.db "SELECT skill, trigger, load_success FROM skill_invocations;"
```
...
```bash
puraguin judge --judge codex --reanalyze all
```
...
```bash
puraguin aggregate
sqlite3 ~/puraguin/state.db "SELECT * FROM skill_stats WHERE window='all';"
```
...
```bash
puraguin report --overview
puraguin report --skill brainstorming
```
...
```bash
puraguin improve --skill brainstorming
cat ~/puraguin/evidence/brainstorming-$(date -u +%Y-%m-%d).md
```

Expected: markdown file with summary, positive/negative exemplars, gap findings, hand-off block.

- [ ] **Step 3: Invoke /puraguin in a fresh Claude Code session**

In a fresh `claude` session, type:

```
/puraguin improve brainstorming
```

Expected: the skill reads the evidence pack, summarises patterns, then invokes `Skill(skill-creator)` or `Skill(writing-skill)` with the evidence pack as input. Apply or reject the resulting rewrite.

- [ ] **Step 4: Mark outcome**

```bash
puraguin improve --skill brainstorming --mark applied
sqlite3 ~/puraguin/state.db "SELECT skill, status FROM skill_improvements;"
```
...
### Task 8.5: Schedule periodic `puraguin run`

- [ ] **Step 1: Decide cadence and pick scheduler**

Options:
- cron (host-level): `0 */6 * * * /usr/bin/env puraguin run --skip-gaps`
- `/loop /puraguin` inside Claude Code (less reliable, requires interactive session)
...
0 */6 * * * /home/ace/.local/bin/puraguin run --skip-gaps >> $HOME/puraguin/cron.log 2>&1
30 3 * * * /home/ace/.local/bin/puraguin run         >> $HOME/puraguin/cron.log 2>&1
EOF
) | crontab -
crontab -l
```
...
```bash
tail ~/puraguin/cron.log
sqlite3 ~/puraguin/state.db "SELECT MAX(last_processed_at) FROM source_files;"
```
...
- A fresh Claude Code session writes events to `~/puraguin/events/claude-code/`.
- `puraguin run` ingests → judges → aggregates with no errors.
- `puraguin report --overview` produces a usable HTML page served on `localhost:8765`.
- `/puraguin improve <skill>` correctly hands off to `Skill(skill-creator)` / `Skill(writing-skill)` rather than rewriting inline.
