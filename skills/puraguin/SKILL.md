...
4. For improve: read the evidence pack file at the printed `evidence_path`, summarise the key patterns (~10 lines), then explicitly invoke `Skill(skill-creator)` or `Skill(writing-skill)` with the evidence pack content as input. Do NOT attempt to rewrite the SKILL.md inline yourself.
...
- Invoke `Skill(skill-creator)` for skills that need structural changes, or `Skill(writing-skill)` for description-only rewrites.
