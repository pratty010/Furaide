...
def _cmd_ingest(args):
    from mekiki import ingest
    counts = ingest.run(platform=args.platform)
    print(json.dumps(counts, indent=2))

def _cmd_judge(args):
    from mekiki.judge import get_backend, orchestrator
...
def _cmd_aggregate(args):
    from mekiki import aggregate
    counts = aggregate.run()
    print(json.dumps(counts, indent=2))

def _cmd_run(args):
    from mekiki import ingest, aggregate, config
    from mekiki.judge import get_backend, orchestrator
    cfg = config.load().judge
...
def _cmd_report(args):
    from mekiki.report import generate, server
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mekiki")
