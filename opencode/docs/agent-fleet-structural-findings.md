...
| kodama--growth-echo.md | subagent | opencode-go/qwen3.6-plus | Growth Echo: Recurring-gap analysis and growth roadmap specialist (handles growth_planning intent). | migaki--profile-polisher, akashi--proof-keeper |  |
...
| kudagitsune--fit-diviner.md | subagent | openai/gpt-5.4-mini | Fit Diviner: Current-state scoring and role-fit judgment specialist for the brand bundle. | kurabokko--knowledge-keeper, migaki--profile-polisher, kudagitsune--fit-diviner |  |
...
| shiranui--migration-guide.md | all | opencode-go/kimi-k2.5 | Migration Guide: Phased migration and codemod orchestrator. |  | mikoshi--code-pathfinder, karakuri--command-runner, tanuki--codemod-runner |
...
| tanuki--codemod-runner.md | subagent | opencode-go/mimo-v2.5 | Codemod Runner: Plans and orchestrates bulk code transformations (jscodeshift, ast-grep, sed-based transforms, API migration fixups) by dispatching bounded shell commands to karakuri--command-runner. | tsukumogami--code-forgemaster | karakuri--command-runner |
...
7. **Brand-bundle sub-familiars all use `openai/gpt-5.2` or `openai/gpt-5.4-mini`.** This is currently a soft cap usage: `gpt-5.2` is the primary for `kudagitsune--fit-diviner`, `kodama--growth-echo`, `akashi--proof-keeper`, `amanojaku--voice-contrarian` (4 subagents); `gpt-5.4-mini` is primary for `hanko--git-seal`, `hyakume--ats-watchman`, `kurabokko--knowledge-keeper` (3 subagents). Neither model is on the reserved cap list (`opencode-go/glm-5.1`, `opencode-go/qwen3.7-max`, `google-vertex/gemini-3.1-pro-preview`, `openai/gpt-5.5`). However, the 4 sub-familiars sharing `gpt-5.2` create a concentration risk: if that model deprecates or rate-limits, the brand-bundle degrades uniformly. Consider diversifying (e.g., `opencode-go/qwen3.6-plus` for `kodama--growth-echo`, `openai/gpt-5.4-mini` for `kudagitsune--fit-diviner`). Out of scope for Task 7; flag for the fleet operator.
   **Status: Approved + Applied (Task 8, partial).** `kodama--growth-echo` primary changed `openai/gpt-5.2` → `opencode-go/qwen3.6-plus`. `kudagitsune--fit-diviner` primary changed `openai/gpt-5.2` → `openai/gpt-5.4-mini`. Both promoted from the `brand_subagents_deferred` list in `docs/routing-manifest.json` and added to `subagents` with the new primaries and a fallback chain spanning 2+ providers. `gpt-5.2` primary count drops from 4 to 2 (still used by `akashi--proof-keeper` and `amanojaku--voice-contrarian`); further diversification of those two is queued for a future task.
