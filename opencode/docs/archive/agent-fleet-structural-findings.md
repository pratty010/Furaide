...
1. **Remove `tsukumogami--code-forgemaster: allow` from `shiranui--migration-guide` `permission.task`.** (Carried over from Task 6 specialist findings.) The presence of this entry currently lets a migration specialist dispatch a coding specialist, which breaks the specialist→subagent-only dispatch rule. Migration execution should reach `tsukumogami` only via the plan handoff (the user invokes `tsukumogami` directly with the migration plan in hand) or via `karakuri--command-runner` for gated shell work.
   **Status: Approved + Applied (Task 8).** Entry removed from `agents/shiranui--migration-guide.md` `permission.task`. Replaced with `karakuri--command-runner: allow` and `tanuki--codemod-runner: allow` (see #2). Routing-manifest `_note` for shiranui updated to reflect the new dispatch chain.
...
2. **Consider a T2 codemod leaf.** If migration work needs bulk renames that exceed `henge--format-shifter`'s scope, a dedicated codemod T2 leaf would be the correct escalation target in place of a specialist dispatch. Out of scope for Task 7 but worth queueing.
   **Status: Approved + Applied (Task 8).** New agent `agents/tanuki--codemod-runner.md` created with `mode: subagent`, `model: opencode-go/mimo-v2.5`, `tier: T2`. `permission.task` is `{"*": deny, "karakuri--command-runner": allow}` so codemod execution routes through gated shell. Added to `scripts/lib/agent-fleet-map.mjs` (39th entry), `docs/routing-manifest.json` subagents, `fleet-manifest.json` agents-core (30 agents), and `agents/shiranui--migration-guide.md` `permission.task`. The new agent is technically a thin worker (it can dispatch to karakuri), not a strict T2 leaf — but the contract holds: it never dispatches further subagents, only karakuri for shell. Integrity test updated from 38 to 39.
...
3. **Body content includes are unresolved for the three common/ specialists.** `chizu--implementation-planner.md`, `shiranui--migration-guide.md`, and `sojobo--system-strategist.md` all reference `../common/agents/.../core.md` includes. The `common/` directory does not exist in this worktree, so the body of these specialists is effectively empty. This is a fleet-merge concern, not an agent-defect concern, but it should be flagged for the merge process. (The integrity tests pass because they validate frontmatter and rename maps, not body resolution.)
   **Status: Acknowledged (Task 8).** Flagged for the merge process but not fixed in this worktree — the `common/` directory is supplied by the merge step, not present in the opencode config dir alone. No code change. The integrity tests pass because they validate frontmatter and rename maps, not body resolution; this is the expected state until the merge.
...
4. **`mujina--brand-shapeshifter` is described as the only lightweight-advisory specialist.** If other specialists grow similarly lightweight siblings, consider a `mode: advisory` value in addition to `primary`/`subagent`/`all`. Out of scope for Task 7; queue as a future structural question.
   **Status: Deferred.** No change. Future structural question, not part of Task 8 scope.
...
5. **Stale body references in brand-bundle sub-familiars.** `kodama--growth-echo` body still says "you are the Brand Builder growth chizu--implementation-planner" (copy-paste artifact) and `amanojaku--voice-contrarian` body says "anti-voice oni--red-team-reviewer" (same issue). These are body-text defects caught during description review. Description frontmatter is now clean (the rubric shape was applied) but the body still has the same misleading internal-handle reference. Recommended: a separate "body cleanup" pass to remove stale internal-handle references. Out of scope for Task 7 (rubric is frontmatter-only) but worth flagging.
   **Status: Approved + Applied (Task 8).** `agents/kodama--growth-echo.md` body now reads "You are the Brand Builder growth-roadmap specialist." `agents/amanojaku--voice-contrarian.md` body now reads "You are the Brand Builder anti-voice gate." Both misleading internal-handle references removed.
...
6. **`tanuki--general-trickster` description now names 12 specialists in `Not for:`.** The list will go stale whenever a new specialist is added. Consider extracting the specialist list to a single source of truth (`docs/routing-manifest.json` or a fleet index) and referencing it. Out of scope for Task 7; queue as a future maintenance question.
   **Status: Approved + Applied (Task 8).** `agents/tanuki--general-trickster.md` `Not for:` now reads "(see the `specialists` section of `docs/routing-manifest.json` for the canonical list) — if one fits, surface the routing and stop." The 12-item inline list is gone; the routing manifest is now the single source of truth.

7. **Brand-bundle sub-familiars all use `openai/gpt-5.4-mini`.** This is currently a soft cap usage: `gpt-5.4-mini` is the primary for `akashi--proof-keeper`, `amanojaku--voice-contrarian`, `hyakume--ats-watchman`, `kurabokko--knowledge-keeper` (4 subagents); `gpt-5.4-mini` is primary for `hanko--git-seal`, `hyakume--ats-watchman`, `kurabokko--knowledge-keeper` (3 subagents). Neither model is on the reserved cap list (`opencode-go/glm-5.1`, `opencode-go/qwen3.7-max`, `google-vertex/gemini-3.1-pro-preview`, `openai/gpt-5.5`). However, the 4 sub-familiars sharing `gpt-5.2` create a concentration risk: if that model deprecates or rate-limits, the brand-bundle degrades uniformly. Consider diversifying (e.g., `opencode-go/qwen3.6-plus` for `kodama--growth-echo`, `openai/gpt-5.4-mini` for `kudagitsune--fit-diviner`). Out of scope for Task 7; flag for the fleet operator.
   **Status: Approved + Applied (Task 8, partial).** `kodama--growth-echo` primary changed `openai/gpt-5.2` → `opencode-go/qwen3.6-plus`. `kudagitsune--fit-diviner` primary changed `openai/gpt-5.2` → `openai/gpt-5.4-mini`. Both promoted from the `brand_subagents_deferred` list in `docs/routing-manifest.json` and added to `subagents` with the new primaries and a fallback chain spanning 2+ providers. `gpt-5.2` was fully replaced with `gpt-5.4-mini` for availability; further diversification of those two is queued for a future task.

## Approval Gate

- Do not apply hierarchy changes until the user approves this file.

## Task 8 application summary

Proposals #1, #2, #3, #5, #6, #7 were applied on 2026-06-05. #4 deferred. File-level diff:

- `agents/shiranui--migration-guide.md` — `permission.task` updated (tsukumogami removed, karakuri + tanuki--codemod-runner added).
- `agents/tanuki--codemod-runner.md` — new file (T2 codemod leaf).
- `scripts/lib/agent-fleet-map.mjs` — 39th entry added.
- `scripts/tests/agent-fleet-integrity.test.mjs` — `expect(AGENT_RENAME_MAP).toHaveLength(38)` updated to 39.
- `docs/routing-manifest.json` — `tanuki--codemod-runner`, `kodama--growth-echo`, `kudagitsune--fit-diviner` added to `subagents`; shiranui `_note` rewritten; `brand_subagents_deferred` note updated.
- `fleet-manifest.json` — `agents-core` description updated to 30 agents; `tanuki--codemod-runner.md` added to the file list.
- `agents/kodama--growth-echo.md` — `model:` updated; body line 25 misleading handle fixed.
- `agents/kudagitsune--fit-diviner.md` — `model:` updated.
- `agents/amanojaku--voice-contrarian.md` — body line 25 misleading handle fixed.
- `agents/tanuki--general-trickster.md` — `Not for:` line now references the routing manifest.
- `docs/agent-fleet-structural-findings.md` — this section.

Test suite: 97/97 pass (up from 94; +3 for the new agent's primary model in routing-manifest).
