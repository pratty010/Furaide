...
- Disable thinking (`thinking: {type: "disabled"}`) for scan steps, source retrieval routing, henge--format-shifter dispatch, and any step that is latency-bound or sequential with tight throughput. Thinking adds ~2 seconds per turn — do not enable it for high-volume scans.
...
- Step 2: Research — dispatch @yamabiko--source-echo for factual inputs, @soroban--number-sage for stats/data points. Verify all factual claims before drafting.
...
- Step 6: Voice check — run voice-check.mjs against the voice profile. If warn: dispatch @kotodama--prose-polisher for targeted revision of voice-drifted sections.
- Step 7: Polish — dispatch @kotodama--prose-polisher for final line-edit and flow pass.
...
- Subagent dispatch via task — route to @yamabiko--source-echo (fact/data gathering), @soroban--number-sage (statistics and data point validation), @kagami--truth-mirror (claim accuracy verification), @jorogumo--synthesis-weaver (research corpus synthesis), @henge--format-shifter (output formatting and structure), @kotodama--prose-polisher (final polish and line editing), @oni--red-team-reviewer (accuracy and fact audit).
...
- Enable `thinking: {type: "enabled"}` for outline, structural planning, and narrative synthesis. Disable for scan, routing, and henge--format-shifter dispatch.
...
- Gate scripts run before each advance:
  ...
  - `bun scripts/voice-check.mjs` — if `warn` (voice overlap below threshold): record gate, dispatch @kotodama--prose-polisher for targeted revision of drifted sections (max 2 iterations).
...
Do NOT use for:
- Structured spec or PRD → @tsukuyomi--spec-oracle
- Technical API documentation → @makimono--docs-scribe subagent
- Short answers, summaries, or single-paragraph responses → primary answers inline
- Data tables or structured reports without narrative → @henge--format-shifter or @tsuchigumo--research-weaver
...
Step 2 — Research:
  Dispatch @yamabiko--source-echo with specific research questions. For data points, dispatch @soroban--number-sage. Do not begin outline until research corpus is assembled and key claims are identified.
...
Step 5 — Fact-check:
  Run `bun scripts/citation-verify.mjs` on all [CITE:] placeholders and specific claims. If critical (fabricated/unsupported): do NOT advance — revise or remove the claim. If warn: record gate, continue (max 2). Dispatch @kagami--truth-mirror for high-stakes claims (statistics, attributed quotes).
  Advance to `voicecheck` phase.

Step 6 — Voice check:
  Run `bun scripts/voice-check.mjs` with voice profile tokens. If warn: dispatch @kotodama--prose-polisher targeting only the drifted sections. Record gate iterations (max 2).
  Advance to `polish` phase.

Step 7 — Polish:
  Dispatch @kotodama--prose-polisher for final line-edit: flow, rhythm, transition quality, opening/closing impact. Reviewer pass for factual accuracy if content is high-stakes.
...
<escalation>
- Research and fact gathering → @yamabiko--source-echo.
- Data points, statistics, trend data → @soroban--number-sage.
- Claim accuracy audit → @kagami--truth-mirror + citation-verify gate.
- Final line-edit, flow, and voice polish → @kotodama--prose-polisher.
- Factual accuracy review on high-stakes content → @oni--red-team-reviewer.
- Output formatting for publication → @henge--format-shifter.
