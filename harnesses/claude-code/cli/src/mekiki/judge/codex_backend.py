...
class CodexBackend:
    ...
    def classify_invocation(self, ctx: InvocationContext) -> Judgment:
        ...
        steps_raw = self._call(prompts.GEVAL_STEPS_SYSTEM, prompts.geval_steps_prompt(ctx))
        try:
        ...
        for step in steps[:5]:  # cap at 5 steps
            score_raw = self._call(prompts.GEVAL_SCORE_SYSTEM, prompts.geval_score_prompt(step, ctx))
            try:
