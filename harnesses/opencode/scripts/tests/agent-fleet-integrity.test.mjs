...
import { AGENT_RENAME_MAP, LEGACY_AGENT_ALIASES, V2_FLEET } from '../lib/agent-fleet-map.mjs';

test('rename map covers the v2 fleet with unique current and target names', () => {
  expect(AGENT_RENAME_MAP).toHaveLength(V2_FLEET.length);

  const current = new Set(AGENT_RENAME_MAP.map(entry => entry.current));
  const target = new Set(AGENT_RENAME_MAP.map(entry => entry.next));

  expect(current.size).toBe(V2_FLEET.length);
  expect(target.size).toBe(V2_FLEET.length);
});

test('legacy alias map points to v2 fleet targets', () => {
...
test('all v2 fleet agent files exist', () => {
  for (const name of V2_FLEET) {
